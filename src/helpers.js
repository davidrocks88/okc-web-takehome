...
export function getRandomTemplate(fieldName) {
  const templates = getTextTemplates(fieldName);
  return templates[Math.floor((Math.random()*templates.length))];
}

export function getFormattedEssayString(template, answer) {
  return [...reactStringReplace(template, "$answer", () => <b key={answer}>{answer}</b>), " "]
}